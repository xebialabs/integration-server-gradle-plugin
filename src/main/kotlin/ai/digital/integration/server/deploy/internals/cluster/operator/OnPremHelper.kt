...
open class OnPremHelper(project: Project): OperatorHelper(project) {
    ...
    fun launchCluster() {
        ...
        createCluster(name, onPremiseProvider.clusterNodeCpus, onPremiseProvider.clusterNodeMemory, kubernetesVersion, skipExisting)
        updateContext(name)
        val kubeContextInfo = getKubectlHelper().getCurrentContextInfo()
        project.logger.lifecycle("kubeContextInfo {}", kubeContextInfo)
    ...
    private fun createCluster(name: String, clusterNodeCpus: Property<Int>, clusterNodeMemory: Property<Int>, kubernetesVersion: String, skipExisting: Boolean) {
        val clusterName = onPremClusterName(name)
