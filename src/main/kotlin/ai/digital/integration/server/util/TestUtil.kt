...
class TestUtil {
    companion object {
        ...
        fun hasTests(project: Project): Boolean {
            return !ExtensionUtil.getExtension(project).tests.isEmpty()
        }

        @JvmStatic
        fun getBaseTest(project: Project): Test? {
            return ExtensionUtil.getExtension(project).tests.find { test -> test.base }
        }

        @JvmStatic
        fun getExecutableTests(project: Project): List<Test> {
            return ExtensionUtil.getExtension(project).tests
                .filter { test ->
