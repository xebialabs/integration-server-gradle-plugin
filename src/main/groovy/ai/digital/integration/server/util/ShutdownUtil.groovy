...
class ShutdownUtil {
    private static void waitForShutdown(Project project) {
        ...
        while (triesLeft > 0 && !success) {
            try {
                def http = HTTPUtil.buildRequest(DeployServerUtil.composeUrl(project, server.contextRoot))
    ...
    static void shutdownServer(Project project) {
        ...
        try {
            ...
            def http = HTTPUtil.buildRequest(DeployServerUtil.composeUrl(project, "/deployit/server/shutdown"))
